...
# This is an optimized version of cons_all2bnet.py, in which many redundant nodes are removed from the Bayesian network.
# While we believe that this improves running times, its impact on accuracy is not entirely clear. While the smaller
# Bayesian network is likely to behave better with belief propagation, the asymmetry between singly-derivable and
# multiply-derivable nodes worries Mukund. Therefore, it has not been turned on.
...
# ./cons_all2bnet-opt.py bnet-dict.out [narrowor] < named_cons_all.txt > named-bnet.out
...
def singleton2Elem(container):
    """Given a container containing a single element, returns the element."""
    assert len(container) == 1
    for elem in container: return elem

def getArbitrary(container):
    """Returns an arbitrary element from a non-empty container."""
...
# "Simple" consequents are those which can be derived in only one way. Non-simple consequents are defined by the
# disjunction of all their deriving clauses, and therefore need a disjunction node. Simple consequents do not need a
# disjunction node, and can be identified with their deriving clauses.

simpleConsequents = { t: singleton2Elem(tuple2ConsequentClauses[t]) for t in allConsequents \
                                                                    if len(tuple2ConsequentClauses[t]) == 1 }
nonSimpleConsequents = { t for t in allConsequents if len(tuple2ConsequentClauses[t]) > 1 }
allNodes = list(allClauses | nonSimpleConsequents)
logging.info('Discovered {0} bayesian nodes.'.format(len(allNodes)))

# 3b. Print dictionary file
nodeIndex = { node: index for index, node in enumerate(allNodes) }
nodeIndex.update({ t: nodeIndex[simpleConsequents[t]] for t in simpleConsequents.keys() })

with open(dictOutFileName, 'w') as dictOutFile:
    for node, index in nodeIndex.items():
        nodeStr = node if node in allTuples else ', '.join(node)
        print('{0}: {1}'.format(nodeIndex[node], nodeStr), file=dictOutFile)
    logging.info('Finished producing dictionary.')

# 3c. Output rules for each node
print(len(allClauses) + len(nonSimpleConsequents))
