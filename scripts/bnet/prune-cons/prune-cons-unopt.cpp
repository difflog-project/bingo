...
int main(int argc, char *argv[]) {
    ...
    bool optimize = true;
    if (argc == 4 && string(argv[3]) == "noopt") optimize = false;
    ...
    unordered_set<string> coreachableTuples;
    if (optimize) {
        coreachableTuples = allOutputTuples;
        ...
        for (const auto& c : allConsequents) {
            coreachableTuples.insert(c);
        }
    }
